...
public abstract class ConfigEdit {
	 ...
	 */
	public record FieldDef(Component label, Kind kind, ModConfigSpec.ConfigValue<?> value,
	                       @Nullable List<Component> fixed) {

		public FieldDef(Component label, Kind kind, ModConfigSpec.ConfigValue<?> value) {
			this(label, kind, value, null);
		}

		public String getString() {
			return switch (kind) {
				case BOOL -> String.valueOf(((ModConfigSpec.BooleanValue) value).get());
				case INT -> String.valueOf(((ModConfigSpec.IntValue) value).get());
				case DOUBLE -> String.valueOf(((ModConfigSpec.DoubleValue) value).get());
				case STRING -> String.valueOf(((ModConfigSpec.ConfigValue<String>) value).get());
			};
		}

		public void set(String s) {
			switch (kind) {
				case BOOL -> ((ModConfigSpec.BooleanValue) value).set(Boolean.parseBoolean(s));
				case INT -> ((ModConfigSpec.IntValue) value).set(Integer.parseInt(s));
				case DOUBLE -> ((ModConfigSpec.DoubleValue) value).set(Double.parseDouble(s));
				case STRING -> ((ModConfigSpec.ConfigValue<String>) value).set(s);
			}
		}
		...
		public void reset() {
			switch (kind) {
				case BOOL -> ((ModConfigSpec.BooleanValue) value).set(((ModConfigSpec.BooleanValue) value).getDefault());
				case INT -> ((ModConfigSpec.IntValue) value).set(((ModConfigSpec.IntValue) value).getDefault());
				case DOUBLE -> ((ModConfigSpec.DoubleValue) value).set(((ModConfigSpec.DoubleValue) value).getDefault());
				case STRING ->
						((ModConfigSpec.ConfigValue<String>) value).set(((ModConfigSpec.ConfigValue<String>) value).getDefault());
			}
		}
		...
		public List<Component> tooltip(ConfigEdit edit) {
			...
			if (vs instanceof ModConfigSpec.ValueSpec vs2) {
		...
		public FormScreen.FormField toFormField(ConfigEdit edit) {
			...
			return switch (kind) {
				...
				case INT -> FormScreen.FormField.text(label, String.valueOf(((ModConfigSpec.IntValue) value).get()),
						edit::validateInt, tip);
				case DOUBLE ->
						FormScreen.FormField.text(label, String.valueOf(((ModConfigSpec.DoubleValue) value).get()),
								edit::validateDouble, tip);
				case STRING -> FormScreen.FormField.text(label, String.valueOf(((ModConfigSpec.ConfigValue<String>) value).get()),
						null, tip);
			};
		}
	}
	...
	protected abstract ModConfigSpec commonSpec();

	protected abstract ModConfigSpec clientSpec();
	...
	private static void saveIfMatch(ModConfig.Type type, ModConfigSpec spec) {
		for (ModConfig c : ModConfigs.getConfigSet(type)) {
			if (c.getSpec() == spec) {
				c.getLoadedConfig().save();
			}
		}
	}
