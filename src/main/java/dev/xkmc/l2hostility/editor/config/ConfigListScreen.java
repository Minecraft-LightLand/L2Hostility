...
public class ConfigListScreen extends ListEditScreen<EntityConfig.Config> {
	...
	private record ConfigHandler(boolean pickEntities) implements Handler<EntityConfig.Config> {
		...
		@Override
			...
			private static EntityConfig.Config newEntityConfig(@Nullable EntityType<?> type) {
				...
				if (type != null) ans.entities = HolderSet.direct(EntityType::builtInRegistryHolder, List.of(type));
				return ans;
			}
