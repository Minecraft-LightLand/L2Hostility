...
public enum HostilityEditorLang {
	...
	CLIENT("editor.client", "Client", 0, null),
	COMMON("editor.common", "Common", 0, null),
	...
	RESET_TIP("editor.tip_reset", "Reset all config values shown here to their defaults.", 0, ChatFormatting.GRAY),
	...
	MASTER("editor.master.title", "Master", 0, null),
	...
	SUMMARY_LV("editor.summary.lv", "lv %s", 1, null),
	SUMMARY_MIN_LV("editor.summary.min_lv", "minLv %s", 1, null),
	SUMMARY_MIN("editor.summary.min", "min %s", 1, null),
	SUMMARY_BASE("editor.summary.base", "base %s", 1, null),
	SUMMARY_VAR("editor.summary.var", "var %s", 1, null),
	SUMMARY_SCALE("editor.summary.scale", "scale %s", 1, null),
	SUMMARY_COST("editor.summary.cost", "cost %s", 1, null),
	SUMMARY_RANK("editor.summary.rank", "rank %s", 1, null),
	SUMMARY_W("editor.summary.w", "w %s", 1, null),
	SUMMARY_ITEMS("editor.summary.items", "%s items", 1, null),
	SUMMARY_ENCHANTS("editor.summary.enchants", "%s enchants", 1, null),
	SUMMARY_ENTRIES("editor.summary.entries", "%s entries", 1, null),
	SUMMARY_TRAITS("editor.summary.traits", "%s traits", 1, null),
	SUMMARY_FREE("editor.summary.free", "free %s", 1, null),
	SUMMARY_CAP("editor.summary.cap", "cap", 0, null),
	SUMMARY_CONFIGS("editor.summary.configs", "%s configs", 1, null),
	SUMMARY_MIN_SPAWN("editor.summary.min_spawn", "minSpawn %s", 1, null),
	SUMMARY_MAX_LV("editor.summary.max_lv", "maxLv %s", 1, null),
	SUMMARY_MAX_TRAIT("editor.summary.max_trait", "maxTrait %s", 1, null),
	SUMMARY_MAX_TOTAL("editor.summary.max_total", "maxTotal %s", 1, null),
	SUMMARY_INTERVAL("editor.summary.interval", "interval %s", 1, null),
	SUMMARY_MINIONS("editor.summary.minions", "%s minions", 1, null),
	SUMMARY_HP("editor.summary.hp", "hp %s", 1, null),
	SUMMARY_ATK("editor.summary.atk", "atk %s", 1, null),
	SUMMARY_HP_SCALE("editor.summary.hp_scale", "hpScale %s", 1, null),
	SUMMARY_ATK_SCALE("editor.summary.atk_scale", "atkScale %s", 1, null),
	SUMMARY_PCT("editor.summary.pct", "%s%%", 1, null),
	STRUCTURE_SERVER_HINT("editor.structure_server_hint", " (singleplayer only)", 0, ChatFormatting.GRAY),
	SUMMARY_PRESET("editor.summary.preset", "preset", 0, null),
	SUMMARY_DEFAULT("editor.summary.default", "(default)", 0, null),
	SUMMARY_APPLY("editor.summary.apply", "apply %s", 1, null),
	SUMMARY_TRAIT_CHANCE("editor.summary.trait_chance", "traitChance %s", 1, null),
	SUMMARY_SUPPRESS("editor.summary.suppress", "suppress %s", 1, null),
	...
	TRAIT_DISABLED_TOGGLE("editor.trait_disabled_toggle", "Disabled: the trait toggle for this trait is off in the config.", 0, ChatFormatting.GRAY),
	TRAIT_DISABLED_LEGENDARY("editor.trait_disabled_legendary", "Disabled: legendary traits are turned off by the global config.", 0, ChatFormatting.GRAY),
	...
	// Managed tag descriptions (shown when hovering a tag file in the Tags tab)
	TAG_INFO_BLACKLIST("editor.tag.info.blacklist", "Entities in this tag never receive levels or traits.", 0, ChatFormatting.GRAY),
	TAG_INFO_WHITELIST("editor.tag.info.whitelist", "Entities in this tag can receive levels. Required for entities that don't have levels by default.", 0, ChatFormatting.GRAY),
	TAG_INFO_DEFAULT_BLACKLIST("editor.tag.info.default_blacklist", "Default entity blacklist for traits. Entities here are excluded from trait rolls.", 0, ChatFormatting.GRAY),
	TAG_INFO_DEFAULT_WHITELIST("editor.tag.info.default_whitelist", "Default entity whitelist for traits. Entities here are preferred trait targets.", 0, ChatFormatting.GRAY),
	TAG_INFO_NO_SCALING("editor.tag.info.no_scaling", "Entities in this tag never get level-based stat scaling.", 0, ChatFormatting.GRAY),
	TAG_INFO_NO_TRAIT("editor.tag.info.no_trait", "Entities in this tag never receive traits.", 0, ChatFormatting.GRAY),
	TAG_INFO_SEMIBOSS("editor.tag.info.semiboss", "Boss-like entities that will be excluded from some traits with special mechanics.", 0, ChatFormatting.GRAY),
	TAG_INFO_EFFECT_IMMUNE("editor.tag.info.effect_immune", "Entities that seem to be immune to effects and thus should not get traits that can only be countered by effects.", 0, ChatFormatting.GRAY),
	TAG_INFO_NO_DROP("editor.tag.info.no_drop", "Entities in this tag drop no hostility loot.", 0, ChatFormatting.GRAY),
	TAG_INFO_HIDE_TRAITS("editor.tag.info.hide_traits", "Hide the trait display above these entities.", 0, ChatFormatting.GRAY),
	TAG_INFO_HIDE_LEVEL("editor.tag.info.hide_level", "Hide the level display above these entities.", 0, ChatFormatting.GRAY),
	TAG_INFO_HIDE_TITLE("editor.tag.info.hide_title", "Hide both level and trait display.", 0, ChatFormatting.GRAY),
	TAG_INFO_ARMOR_TARGET("editor.tag.info.armor_target", "Entities in this tag can be equipped with armor by hostility.", 0, ChatFormatting.GRAY),
	TAG_INFO_MELEE_WEAPON_TARGET("editor.tag.info.melee_weapon_target", "Entities in this tag can hold melee weapons.", 0, ChatFormatting.GRAY),
	TAG_INFO_RANGED_WEAPON_TARGET("editor.tag.info.ranged_weapon_target", "Entities in this tag can hold bows/crossbows.", 0, ChatFormatting.GRAY),
	TAG_INFO_SPAWNER_BLACKLIST("editor.tag.info.hostility_spawner_blacklist", "Entities in this tag cannot be spawned by the hostility spawner.", 0, ChatFormatting.GRAY),

	// Short names of the managed tag files (shown as the file row label in the Tags tab)
	TAG_NAME_BLACKLIST("editor.tag.blacklist", "Blacklist", 0, null),
	TAG_NAME_WHITELIST("editor.tag.whitelist", "Whitelist", 0, null),
	TAG_NAME_DEFAULT_BLACKLIST("editor.tag.default_blacklist", "Default Blacklist", 0, null),
	TAG_NAME_DEFAULT_WHITELIST("editor.tag.default_whitelist", "Default Whitelist", 0, null),
	TAG_NAME_NO_SCALING("editor.tag.no_scaling", "No Scaling", 0, null),
	TAG_NAME_NO_TRAIT("editor.tag.no_trait", "No Traits", 0, null),
	TAG_NAME_SEMIBOSS("editor.tag.semiboss", "Semi-Boss", 0, null),
	TAG_NAME_EFFECT_IMMUNE("editor.tag.effect_immune", "Effect Immune", 0, null),
	TAG_NAME_NO_DROP("editor.tag.no_drop", "No Loot", 0, null),
	TAG_NAME_HIDE_TRAITS("editor.tag.hide_traits", "Hide Traits", 0, null),
	TAG_NAME_HIDE_LEVEL("editor.tag.hide_level", "Hide Level", 0, null),
	TAG_NAME_HIDE_TITLE("editor.tag.hide_title", "Hide Title", 0, null),
	TAG_NAME_ARMOR_TARGET("editor.tag.armor_target", "Armor Targets", 0, null),
	TAG_NAME_MELEE_WEAPON_TARGET("editor.tag.melee_weapon_target", "Melee Weapon Targets", 0, null),
	TAG_NAME_RANGED_WEAPON_TARGET("editor.tag.ranged_weapon_target", "Ranged Weapon Targets", 0, null),
	TAG_NAME_SPAWNER_BLACKLIST("editor.tag.hostility_spawner_blacklist", "Spawner Blacklist", 0, null),
