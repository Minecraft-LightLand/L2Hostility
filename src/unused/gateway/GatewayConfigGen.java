...
public class GatewayConfigGen extends PlaceboCodecDataGen<Gateway> {
	...
	public static void genConfig(ConfigDataProvider.Collector collector) {
		for (var e : GATEWAYS) {
			...
			collector.add(L2Hostility.ENTITY, new ResourceLocation(Gateways.MODID,
	...
	public static void genRecipe(RegistrateRecipeProvider pvd) {
		for (var e : GATEWAYS) {
			RecipeGen.unlock(pvd, ShapedRecipeBuilder.shaped(RecipeCategory.MISC,
							GatewayObjects.GATE_PEARL.get(), 1)::unlockedBy, LCItems.VOID_EYE.get())
					.pattern("1A1").pattern("ABA").pattern("1A1")
					.define('B', Items.ENDER_PEARL)
					.define('A', LHItems.HOSTILITY_ESSENCE)
					.define('1', e.mat())
					.save(x -> ConditionalRecipeWrapper.mod(pvd, Gateways.MODID)
